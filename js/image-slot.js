...
(() => {
  ...
  // ── External read/subscribe API ─────────────────────────────────────────
  // Lets sibling scripts (e.g. the desktop OS) derive folder covers from the
  // images dropped inside a folder, and react whenever any slot changes.
  window.ImageSlots = {
    get: (id) => getSlot(id),                              // {u,s,x,y} | null
    subscribe: (fn) => { subs.add(fn); return () => subs.delete(fn); },
    whenLoaded: () => load(),                              // resolves after sidecar hydrates
    isLoaded: () => loaded,
  };
